...
| T-07 | Geofenced notifications only after consent | M6 | `states.ts` holds the consent rule; `Explore.tsx` fires the offer on a **simulated** trigger once per island. **No consent switches in the web app yet**, and no real geofence | `states.test.ts` (both flags required) | **partial** — the offer surface is built and labelled simulated; the consent gate that must precede it is not |

...
function Frond({ frond, index }: { frond: Frond; index: number }) {
  const leaflets = [];
...
export function PalmFronds({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 390 300"
    ...
    >
        ...
        {FRONDS.map((f, i) => {
          ...
          return (
            ...
            <linearGradient
              ...
              y2={tip.y}
            >
              <stop offset="0%" stopColor="#47b487" stopOpacity="0.72" />
              <stop offset="46%" stopColor="#2f9a72" stopOpacity="0.5" />
              <stop offset="100%" stopColor="#1d7355" stopOpacity="0.1" />
            </linearGradient>
          );
        })}
      </defs>
      ...
      {FRONDS.map((f, i) => (
        <Frond key={f.angle} frond={f} index={i} />
      ))}
    </svg>
  );
}

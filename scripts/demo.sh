...
# 8081 is the usual reason the next run mysteriously fails.
set -euo pipefail
...
for port in "$MOBILE_PORT" "$VENDOR_PORT"; do
...
echo "==> Starting the tourist app on http://localhost:$MOBILE_PORT (first bundle takes ~1 minute)"
$PNPM --filter @cvip/mobile web --port "$MOBILE_PORT" >/tmp/cvip-mobile.log 2>&1 &
pids+=($!)
